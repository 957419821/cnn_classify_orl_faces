...
* getData(filedir) 被文件夹外的get_and_restore_data_into_csv.py模块调用</br></br>
get_and_restore_data_into_csv.py 调用上面提到的模块，并将得到的经过提升之后的数据存入csv文件</br></br>
network.py中是简单的CNN，它将调用dataset.py模块，并使用其类得到数据</br></br>
dataset.py模块中包含：
